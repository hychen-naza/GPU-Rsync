...
inline __device__ bool d_char_compare(char *c1, char *c2){
  if(c1[0]!=c2[0] || c1[1]!=c2[1] || c1[2]!=c2[2] || c1[3]!=c2[3]) return false;
  else if(c1[4]!=c2[4] || c1[5]!=c2[5] || c1[6]!=c2[6] || c1[7]!=c2[7]) return false;
  else if(c1[8]!=c2[8] || c1[9]!=c2[9] || c1[10]!=c2[10] || c1[11]!=c2[11]) return false;
  else if(c1[12]!=c2[12] || c1[13]!=c2[13] || c1[14]!=c2[14] || c1[15]!=c2[15]) return false;
  else return true;
}
  ...
  if(fileBeginPos < file_len){
    ...
    for(; i < chunk_size*chunk_num;){
      ...
      if(np == NULL){
        ...
        printf("thread %d need to calcu md5\n",thread_id);
        char sum2[16];
