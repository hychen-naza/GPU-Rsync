...
static struct option gLongOptions[] = {
  ...
  {"task_num",  required_argument,      NULL,           'o'},
 ...
 public:
  ...
  void SendFileInfo(const std::vector<ComponentInfo>& query, const std::string fileName) {
    ...
    for (int i = 0; i < query.size(); ++i) {
      ...
      item->set_checksum2((char*)query[i].checksum2); 
    }
  ...
  void AsyncCompleteRpcCalcu() {
    ...
    while (cqcalcu_.Next(&got_tag, &ok)) {
        ...
        if (call->status.ok()){
          handled_task_num++;
          if(handled_task_num == task_num){
            gettimeofday(&end,NULL);
            time_use=(end.tv_sec-start.tv_sec)*1000000+(end.tv_usec-start.tv_usec);//us nearly 40000us
            std::cout << task_num << " rsync file with size spend " << time_use << " us on computation" << std::endl;
          }
...
int main(int argc, char** argv) {
  ...
  int chunkSize = 256;
  int option_char = 0;
  while ((option_char = getopt_long(argc, argv, "i:s:o:h", gLongOptions, NULL)) != -1) {
    switch (option_char) {
      default:
...
void CpuRsync(int fd, size_t fileSize, int chunkSize, std::vector<ComponentInfo> &vec){
  ...
  while(chunk_id <= chunk_num){
      ...
      memcpy(component.checksum2, sum2, 8);
