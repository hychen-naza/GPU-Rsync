...
class RsyncServiceImpl final : public Rsync::Service {
  ...
  Status CalcuDiff(ServerContext* context, const FileInfo* request,
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout<< "the " << cur_num << "th task spend " << c_time_use << " us allocate ht memory" << std::endl;
    ...
    for (int i = 0; i < size; i++) {
      ...
      uint8_t checksum2[8];
      if(chunkId == 46 || chunkId == 104 || chunkId == 115 ){
        for(int k=0;k<8;++k){
          printf("before insert id %d, before copy chunk md5 %d, original md5 %d\n", chunkId, item.checksum2()[k], checksum2[k]);
          checksum2[k] = (uint8_t)item.checksum2()[k];
        }
      }
      else memcpy(checksum2, (uint8_t*)&item.checksum2()[0], 8); 
      if(chunkId == 46 || chunkId == 104 || chunkId == 115 ){
        for(int k=0;k<8;++k){
          printf("before insert id %d, after copy chunk md5 %d, new md5 %d\n", chunkId, item.checksum2()[k], checksum2[k]);
        }   
      }  
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout<< "the " << cur_num << "th task spend " << c_time_use << " us receive package" << std::endl;
    ...
    for(int i=recalcu_region_size-1; i<t.totalThreads; i += recalcu_region_size){
        ...
        if(t_match_num > 0 && t.stat[j] > 0 && jump_pos > t.matchOffset[chunk_num*j]){
          std::cout << "thread " << i << " need recalcu, jump to " << jump_pos << std::endl;
