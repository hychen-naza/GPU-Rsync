          ...
          int *match_offset, int *match_chunkid, int *stat)
{
  ...
  if(fileBeginPos < file_len){
    ...
    for(; i < chunk_size*chunk_num;){
      ...
      if(thread_id == 46 || thread_id == 104 || thread_id == 115){
        if(i==0) printf("in gpu, thread %d, init jump time is %d\n", thread_id, jump_time);
      }
      if(np.chunk_id == -1){
        recalcu = 0;
        i += 1;
        ...
        for(int j = jump_time;;++j){   
          ...
          np = ht[index3];
          if(thread_id == 46 || thread_id == 104 || thread_id == 115){
            printf("in gpu, thread %d, rc %d, jump time is %d, ht index %d, np chunk id %d\n", thread_id, (int)rc, j, index3, np.chunk_id);
            for(int k=0;k<8;++k){
              printf("thread %d, chunk md5 %d, ht md5 %d\n",thread_id, sum2[k], np.md5[k]);
            }
            if(d_char_compare(sum2,np.md5)){
              printf("in thread %d, md5 match\n",thread_id);
            }
            else{
              printf("in thread %d, md5 not match\n",thread_id);
            }
          }
