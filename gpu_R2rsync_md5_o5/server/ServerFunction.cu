...
class RsyncServiceImpl final : public Rsync::Service {
  ...
  Status CalcuDiff(ServerContext* context, const FileInfo* request,
    ...
    for (int i = 0; i < size; i++) {
      ...
      uint8_t checksum2[8];
      memcpy(checksum2, item.checksum2().c_str(), 8);      
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout << "on server , single rsync file with size " << t.fileLen << ", spend " << c_time_use << " us on gpu calcu" << std::endl;
    /*gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout << "on server , single rsync file with size " << t.fileLen << ", spend " << c_time_use << " us on gpu + gpu recalcu" << std::endl;
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout << "on server , rsync file size " << t.fileLen << ", spend " << c_time_use << " us on sending back to client and free gpu memory" << std::endl;
    return Status::OK;
  }
