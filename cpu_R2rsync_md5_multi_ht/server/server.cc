...
int CpuRsync(HashTable *ht, char *file, int *match_chunkid, int *match_offset, int file_len, int chunk_size){
  ...
  while(offset < file_len){
    ...
    Node *np = lookup_hashtable(ht, rolling_checksum, first_index, second_index, jump_time);
    ...
    // 刚返回的np.rc是相等的，但之后的rc就不一定了，并且md5还不知道等不等
    else{       
      static char sum2[16];
...
class RsyncServiceImpl final : public Rsync::Service {
  ...
  Status CalcuDiff(ServerContext* context, const FileInfo* request,
    ...
    std::cout<< "the " << handled_task_num+1 << "th task spend " << c_time_use << " us receive package from client" << std::endl;
    ...
    std::cout << "matchTokens num " << matchTokens.size() << std::endl;
    ...
    handled_task_num++;
    if(handled_task_num == total_task_num){
