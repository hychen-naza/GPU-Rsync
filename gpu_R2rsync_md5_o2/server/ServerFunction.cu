...
class RsyncServiceImpl final : public Rsync::Service {
  ...
  Status CalcuDiff(ServerContext* context, const FileInfo* request,
    ...
    cudaFree(t.dStat);
    free(t.ht);
    buildTask bt = {t.fileName, t.chunkSize, t.totalThreads, t.fd, t.dFileContent, t.file, t.stat, t.matchChunkid, t.matchOffset, matchIdArray, matchIdMap, t.totalChunkNum, size};
  ...
  Status BuildNewFile(ServerContext* context, const UnmatchChunks* request,
    ...
    cudaMemset(d_new_file, 0, sizeof(char)*new_file_len);
    int *dmatchIdArray;
    ...
    oldfile_match_build<<<nblocks, nthreads, 0, stream[i]>>>(t.dFileContent, dmatchIdArray, t.chunkSize, chunk_num, t.matchOffset, t.matchChunkid, d_new_file, t.totalOldChunkNum, d_multi_match_array, d_multi_match_num);
    cudaMemcpyAsync(new_file, d_new_file, sizeof(char)*new_file_len,cudaMemcpyDeviceToHost,stream[i]);
