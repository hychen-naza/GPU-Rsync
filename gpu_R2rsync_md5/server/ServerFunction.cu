...
class RsyncServiceImpl final : public Rsync::Service {
  ...
  Status CalcuDiff(ServerContext* context, const FileInfo* request,
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout<<"on server, spend " << c_time_use << " us on copying ht into gpu" << std::endl;
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout << "on server , single rsync file with size " << t.fileLen << ", spend " << c_time_use << " us on gpu computation and memory copy" << std::endl;
    ...
    cudaFree(t.dHt);
    cudaFree(t.dStat);      
    for(int i=0;i<t.totalThreads-1;++i){
        int t_match_num = t.stat[i];
        int j = i+1;       
        //std::cout << "thread " << i << " jump to " << t.matchOffset[chunk_num*i+t_match_num-1]+chunk_size << ", thread " << j << " begin from " << t.matchOffset[chunk_num*j] << std::endl;
        ...
        if(t_match_num > 0 && t.stat[j] > 0 && t.matchOffset[chunk_num*i+t_match_num-1]+chunk_size > t.matchOffset[chunk_num*j]){   
          //std::cout << "thread " << i << " need recalcu" << std::endl;   
          ...
          recalcu(chunk_size, chunk_num, t.stat, jump_pos, t.fileLen, t.totalThreads,
                t.file, t.matchOffset, t.matchChunkid, t.ht, j);    
        }    
        /*if(i==267 || i==266){
            for(int k=0;k<chunk_num;++k){
                std::cout << "thread " << i << "chunk id " << t.matchChunkid[i*chunk_num+k] << std::endl;
            }
        }*/  
    }
    ...
    gettimeofday(&c_end,NULL);
    c_time_use=(c_end.tv_sec-c_start.tv_sec)*1000000+(c_end.tv_usec-c_start.tv_usec);
    std::cout << "on server , rsync file size " << t.fileLen << ", spend " << c_time_use << " us on cpu revise" << std::endl;
