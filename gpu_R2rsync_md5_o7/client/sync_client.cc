  ...
  void SendUnmatchChunk(int fd, int match_chunk_size, int chunk_size, std::string filename, int fileSize, MatchTokenReply &reply){
    ...
    for (int i = 0; i < match_chunk_size; i++) {
      ...
      while(chunkId != last_chunk_id+1){
        ...
        std::cout << "client need to send chunk id " << last_chunk_id << std::endl;
...
void CpuRsync(int fd, size_t fileSize, int chunkSize, std::vector<ComponentInfo> &vec){
  ...
  while(chunk_id <= chunk_num){
      ...
      //std::cout << "length " << read_size << ", rc " << rolling_checksum << std::endl;
      uint8_t sum2[8];
      get_checksum2((uint8_t*)chunk_buffer, (size_t)read_size, (uint8_t*)sum2);      
      ComponentInfo component = {chunk_id, rolling_checksum}; 
      component.checksum2 = ((unsigned long)sum2[7]<<56)|((unsigned long)sum2[6]<<48)|((unsigned long)sum2[5]<<40)|((unsigned long)sum2[4]<<32)|((unsigned long)sum2[3]<<24)|((unsigned long)sum2[2]<<16)|((unsigned long)sum2[1]<<8)|sum2[0];

      if(chunk_id == 1972 || chunk_id == 1973 || chunk_id == 1974 || chunk_id == 1975){
        printf("chunk id %d, rc %d, component.checksum2 %lu, sum2 ", chunk_id, (int)rolling_checksum, component.checksum2);
        for(int k=0;k<8;++k){
          printf("%d",sum2[k]);
        }
        if(chunk_id == 1974){
          printf("this is chunk 1974\n");
          printf("CONTENT is ");
        }
        printf("\n");
        printf("CONTENT is ");
        for(int k=0;k<chunkSize;++k){
          std::cout << chunk_buffer[k];
          //printf("%c",);
        }
        printf("\n\n");
      }
